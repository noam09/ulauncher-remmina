...
def find_executable(executable_name):
    # Use shutil.which if available (Python 3.3+)
    if hasattr(shutil, "which"):
        return shutil.which(executable_name)
    try:
        from distutils import spawn

        return spawn.find_executable(executable_name)
    except ImportError:
        return None


# Example usage
executable_path = find_executable("remmina")
...
default_paths = [
    "{}/.local/share/remmina".format(os.environ.get("HOME")),
...
class RemminaExtension(Extension):
    ...
    def list_profiles(self, query):
        ...
        for p in profiles:
            ...
            if (
                (query.lower() in base.lower())
                or (query.lower() in title.lower())
                or all(x.lower() in desc.lower() for x in keywords)
            ):
...
class KeywordQueryEventListener(EventListener):
    def on_event(self, event, extension):
        ...
        if extension.preferences["profiles"] != "" or not remmina_profiles_path:
            # Tilde (~) won't work alone, need expanduser()
            remmina_profiles_path = os.path.expanduser(
                extension.preferences["profiles"]
            )
...
def create_item(name, icon, keyword, description, on_enter):
    return ExtensionResultItem(
        ...
        on_enter=ExtensionCustomAction({"id": on_enter}),
    )
        ...
        with open(profile_path, "r") as f:
            ...
            for line in lines:
                ...
                if line.startswith("protocol="):
                    ...
                    pass
            if len(username) > 0:
                server = "{username}@{server}".format(username=username, server=server)
            if len(proto) > 0:
                server = "{proto}://{server}".format(proto=proto, server=server)
            if len(group) > 0:
                group = " | {group}".format(group=group)
            # Final description string
            desc = "{server} {group}".format(server=server, group=group)
